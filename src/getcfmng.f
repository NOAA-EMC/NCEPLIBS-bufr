...
C   EXCEPTIONS TO THIS RULE ARE FOR ORIGINATING CENTERS AND SUBCENTERS,
C   SINCE THOSE CAN ALSO BE CONTAINED WITHIN THE IDENTIFICATION SECTION
C   (SECTION 1) OF A BUFR MESSAGE.  IN ANY CASE, IF THE SEARCH IS
C   UNSUCCESSFUL, AND IF THERE WAS NO OPTIONAL SECOND MNEMONIC AND
C   ASSOCIATED VALUE SPECIFIED ON INPUT, THE ROUTINE WILL RE-SEARCH THE
C   TABLE TO CHECK WHETHER THE MEANING OF THE FIRST MNEMONIC AND
C   ASSOCIATED VALUE MAY INDEED DEPEND ON THE VALUE OF ONE OR MORE OTHER
C   POSSIBLE SECOND MNEMONICS.  IF SO, THOSE POSSIBLE MNEMONICS ARE
C   RETURNED ALONG WITH A SPECIAL RETURN CODE SO THAT THE CALLING
C   ROUTINE MAY EXAMINE THEM AND POSSIBLY ISSUE ANOTHER SUBSEQUENT CALL
C   TO THIS SAME ROUTINE WITH SPECIFIED VALUES FOR THE SECOND MNEMONIC
C   AND ASSOCIATED VALUE.
C
C PROGRAM HISTORY LOG:
C 2017-01-11  J. ATOR    -- ORIGINAL AUTHOR
C
...
C*	handling for originating centers and subcenters, since those
C*	can be reported in Section 1 of a BUFR message as well as in
C*	Section 3, so if a user requests those mnemonics we can't
C*	necessarily assume they came from within Section 3.
