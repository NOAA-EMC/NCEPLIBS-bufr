...
C> BUFR message that was read, in format of either YYMMDDHH or
C> YYYYMMDDHH, depending on the most recent call to subroutine datelen().
C> @returns ireadmg - integer: Return code:
C> - 0 new BUFR message was successfully read into internal arrays.
C> - -1 there are no more BUFR messages in the file connected to
