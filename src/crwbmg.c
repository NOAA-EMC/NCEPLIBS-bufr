...
/** Maximum length of a filename, including any directory prefixes or
...
/**
 * Read a specified number of bytes from the file that was opened via
 * the most recent call to function cobfl() with io = 'r'.
 *
 * @param bmg - Array containing the newbytes bytes that were read,
 * beginning at byte number isloc.
 * @param mxmb - Number of elements in bmg array; used by the function
 * to ensure that it doesn't overflow the array.
 * @param isloc - Starting byte within bmg into which to read the next
 * newbytes bytes
 * @param newbytes - Number of bytes to read from file most recently
 * opened for input/reading via function cobfl()
 *
 * @returns
 * - 0 normal return
...
/**
 * Open a new file for reading or writing BUFR messages via a C
 * language interface.
 *
 ...
 *    not required when using this function.
 *
 *  - When using this function, it is only possible to have at most
 *    one input (io = 'r') file and one output (io = 'w') file open at
 *    a time. If a successive call to this function is made in either
 *    case where a file of that type is already open, then the
 *    function will automatically close the previous file of that type
 *    before opening the new one.
 *
 *  - When opening a file for input/reading using openbf(), the user
 *    can make subsequent calls to any of the NCEPLIBS-bufr
 *    [message-reading subroutines](@ref hierarchy) to read individual
 *    BUFR messages from that file into internal arrays, followed by
 *    subsequent calls to any of the NCEPLIBS-bufr [subset-reading
 *    subroutines](@ref hierarchy)) to read individual data subsets
 *    from each such message.  However, when opening a file for
 *    input/reading using this function, the user must instead make
 *    subsequent calls to crbmg() to read individual BUFR messages
 *    from that file, and each such message will be returned directly
 *    to the user within an allocated memory array.  The user may
 *    then, if desired, make subsequent calls to readerme() to store
 *    each such message into the same internal arrays, followed by
 *    subsequent calls to any of the NCEPLIBS-bufr [subset-reading
 *    subroutines](@ref hierarchy) to read individual data subsets
 *    from each such message.
 *
 *  - When opening a file for output/writing using openbf(), the user
 *    can make subsequent successive calls to writsb() to pack each
 *    completed data subset into the BUFR message that is currently
 *    open within the internal arrays, for eventual output to that
 *    file.  However, when opening a file for output/writing using
 *    this function, the user can instead, if desired, make subsequent
 *    successive calls to writsa() to pack each completed data subset
 *    into the BUFR message that is currently open within the internal
 *    arrays. The use of writsa() will cause each completed BUFR
 *    message to be returned directly to the user within an allocated
 *    memory array, which in turn can then be written directly to the
 *    file via a subsequent call to cwbmg().
 *
 * Any errors encountered when using this function are automatically
 * logged to standard output, or to an alternate location previously
 * specified via a call to subroutine errwrt().
 *
...
/**
 * Read the next BUFR message from the file that was opened via the
...
/**
 * Write a BUFR message to the file that was opened via the most
 * recent call to function cobfl() with io = 'w'.
 *
 * This function is designed to be easily callable from application
 * program written in either C or Fortran.
 *
...
/**
 * Close all files that were opened via previous calls to function
 * cobfl().
 *
 * @author J. Ator  @date 2005-11-29
 */
