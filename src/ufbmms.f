...
C> Read a specified data subset from internal arrays.
C>
...
C> read into scope for further processing, counting from the beginning of the
C> internal arrays in memory.
C> @param[in] ISUB - integer: Number of data subset to be
C> read from the (IMSG)th BUFR message, counting from the beginning of the message.
C> @param[out] SUBSET - character*8: Table A mnemonic for type of
C> (IMSG)th BUFR message (see [DX BUFR Tables](@ref dfbftab) for
C> further information about Table A mnemonics).
C> @param[out] JDATE - integer: Date-time stored within Section 1 of
C> (IMSG)th BUFR message, in format of either YYMMDDHH or YYYYMMDDHH, depending 
C> on the most recent call to subroutine datelen().
C>
C> @author J. Woollen @date 1994-01-06
