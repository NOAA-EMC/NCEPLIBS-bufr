...
C> messages.
C>
C> @author J. Ator @date 2009-04-21

C> Specify a custom location for
C> the logging of error and diagnostic messages generated by the
C> NCEPLIBS-bufr software.
C>
C> By default, all such messages are printed to standard output.
...
C> the default version of this subroutine contained within the NCEPLIBS-bufr
C> distribution package. The user does not need to explicitly call
C> this subroutine from within the application code, since the NCEPLIBS-bufr
