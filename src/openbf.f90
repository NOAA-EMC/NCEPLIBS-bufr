!> @file
!> @brief Connect a new system file to the BUFRLIB software for
!> reading or writing BUFR messages.

!> This subroutine connects a new file to the BUFRLIB software for
...
!>                       file (unless IO is set to 'FIRST' or 'QUIET', in
!>                       which case this is a dummy argument)
!> @param[in] IO      -- character*(*): flag indicating how LUNIT is to be
...
!>                 -  'SEC3' = same as 'IN', except use Section 3 of input 
!>                              messages for decoding rather than DX BUFR
!>                              table information from LUNDX; in this case
!>                              LUNDX is ignored, and user must provide 
!>                              appropriate [master BUFR tables](@ref dfbfmstab)
!>                              within the directory specified by a subsequent
!>                              call to subroutine mtinfo()
!>                 -  'NODX' = same as 'OUT', except don't write DX BUFR
!>                             table messages to LUNIT
!>                 -   'APN' = same as 'NODX', except begin writing at end
!>                             of file ("append")
...
!>                 -  'INUL' = same as 'IN', except don't read any
!>                             messages whatsoever from LUNIT (e.g. when
!>                             subroutine readerme() is to be used)
!>                 - 'QUIET' = LUNIT is ignored; this is an indicator
!>                             that the value for IPRT in COMMON block
!>                             /QUIET/ is being reset to the value in
!>                             LUNDX
!>                 - 'FIRST' = LUNIT and LUNDX are ignored; this is an
!>                             indicator to initialize the BUFRLIB
!>                             software, in case this subroutine was
!>                             never previously called
!> @param[in] LUNDX   -- integer:
!>                 - If IO is not set to 'FIRST' or 'QUIET' =
!>                   Fortran logical unit number
!>                   containing DX BUFR table information to be used in
!>                   reading/writing from/to LUNIT (depending on the case).
!>                   This value may be set equal to LUNIT if DX BUFR table
!>                   information is already embedded in LUNIT.
!>                 - If IO is set to 'QUIET' = indicator for degree of
!>                   printout:
!>                      - -1 = no printout except for ABORT messages
!>                      -  0 = limited printout (default)
!>                      -  1 = all warning messages are printed out
!>                      -  2 = all warning and informational messages are
!>                             printed out
!>
