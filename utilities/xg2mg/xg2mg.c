...
/************************************************************************
**	Extracts each GRIB2 message from a given input file into its	*
**	own separate output file.					*
**									*
**	NOTE:  Could not use "strstr" to locate "GRIB2" and "7777",	*
...
main( int argc, char *argv[] )
{
	...
	**      -g	saves any GTS bulletin information associated with each GRIB2 message      
	*/
		...
		case 'v':
		    ...
		    printf( "This is xg2mg v1.0.0, built with BUFRLIB v%s\n", bvstr );
		    return 0;
                case 'g':
		    save_GTSbull = 1;
                    break;
	...
	*/
	if ( (optind+1) != argc ) {
	    ...
	    printf( "    -v\n" );
	    printf( "    -g\n" );
	    printf( "  and infile is [path/]filename of input file containing one or more GRIB2 messages\n\n" );
	    return -1;
	}
	...
	*/
	...
	/*
	**  Locate each GRIB2 message within the input file and write each one to a separate output file.
	*/
	noutfile = 0;
	ipid = (int) getpid();
	pmsg = psb = pc;
...
*/
	    sprintf( outfile, "xg2mg.out.%d.%06lu", ipid, ++noutfile );
...
/*
**	    Read the GRIB2 message length from Section 0.
*/
	    memcpy( &wkint, ( pmsg + 8 ), 8 );
	    msglen = iupb( &wkint, &c1, &c64 );

/*
**	    Write the GRIB2 message to the output file.
*/
