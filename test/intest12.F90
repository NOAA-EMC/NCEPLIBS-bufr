...
program intest12
  ...
  do ii = 1, 3
      if ( ireadmg ( 21, cmgtag, idate ) /= 0 ) stop 2
  enddo
  icnt = nmsub (21)
  if ( icnt /= 215 ) stop 3
  ...
  do ii = 1, icnt
      errstr_len = 0
      if ( ii == icnt ) call openbf ( 21, 'QUIET', 2 )
      if ( ireadsb ( 21 ) /= 0 ) stop 4
      if ( ii == icnt ) then
          call openbf ( 21, 'QUIET', 0 )
          if ( ( index( errstr(1:errstr_len), ':INVN:NBMP:TAG(INODE(LUN)) =   1:     66:  431:NC021206' ) == 0 ) .or. &
               ( index( errstr(1:errstr_len), ':NEWN:NBMP:NVAL(LUN) = (CRCHNM)  :    3:  431' ) == 0 ) ) stop 5
      endif
  enddo

  ! Try to read the first subset of the next message, which should trigger a different errwrt branch.
  errstr_len = 0
  if ( ireadmg ( 21, cmgtag, idate ) /= 0 ) stop 6
  call readsb ( 21, iret )
  if ( ( iret /= -1 ) .or. ( igetsc ( 21 ) /= 1 ) .or. &
       ( index( errstr(1:errstr_len), 'USRTPL - REPLICATION FACTOR OVERFLOW' ) == 0 ) ) stop 7
