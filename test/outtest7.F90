...
module Share_errstr_outtest7
  ...
  integer errstr_len
end module Share_errstr_outtest7
...
program outtest7
  ...
  if ( ( icnt1 /= 1 ) .or. ( icnt2 /= 3 ) .or. &
       ( imesg(1) /= 5 ) .or. ( imesg(2) /= 8 ) .or. ( imesg(3) /= 2 ) .or. ( imesg(4) /= 0 ) ) stop 2

  ! Test some errwrt cases in ufbrms.
  errstr_len = 0
  call ufbrms ( 4, 1, r8vals, -2, 5, nlv, 'SWDE A2CFDFS' )
  if ( ( index( errstr(1:errstr_len), 'UFBRMS - 4th ARG. (INPUT) IS .LE. 0' ) == 0 ) ) stop 3
  errstr_len = 0
  call ufbrms ( 4, 1, r8vals, 2, -5, nlv, 'SWDE A2CFDFS' )
  if ( ( index( errstr(1:errstr_len), 'UFBRMS - 5th ARG. (INPUT) IS .LE. 0' ) == 0 ) ) stop 4

  ! Check some specified values within the 1st subset of the 4th message.
  call ufbrms ( 4, 1, r8vals, 2, 5, nlv, 'SWDE A2CFDFS' )
  if ( ( nlv /= 5 ) .or. &
      ( nint(r8vals(1,1)*10000000) /= 325011000 ) .or. ( nint(r8vals(1,2)*10000000) /= 264548000 ) .or. &
      ( nint(r8vals(1,3)*10000000) /= 419641000 ) .or. ( nint(r8vals(1,4)*10000000) /= 289883000 ) .or. &
      ( nint(r8vals(1,5)*10000000) /= 117109000 ) .or. ( nint(r8vals(2,1)*10000) /= 6140 ) .or. &
      ( nint(r8vals(2,2)*10000) /= 5909 ) .or. ( nint(r8vals(2,3)*10000) /= 7302 ) .or. &
      ( nint(r8vals(2,4)*10000) /= 7413 ) .or. ( nint(r8vals(2,5)*10000) /= 4968 ) ) stop 5
  ...
  if ( ( cmgtag /= 'NC008032' ) .or. ( idate /= 22053116 ) .or. ( nlv /= 1 ) .or. &
      ( nint(r8vals(1,1)*100000) /= 4081139 ) .or. ( nint(r8vals(2,1)*100000) /= -7787666 ) ) stop 6

  ! Copy the 3rd message to the output file.
  call rdmemm ( 3, cmgtag, idate, ier )
  if ( ( ier /= 0 ) .or. ( cmgtag /= 'NC002104' ) ) stop 7
  call cpymem ( 50 )
  ...
  if ( ireadmg ( 21, cmgtag, idate ) /= 0 ) stop 8
  ...
  if ( ( iostat1 /= 0 ) .or. ( iostat2 /= 0 ) ) stop 10
