...
program outtest7
  ...
  if ( ( index( errstr(1:errstr_len), 'UFBRMS - 4th ARG. (INPUT) IS .LE. 0' ) .eq. 0 ) ) stop 3
  errstr_len = 0
  call ufbrms ( 4, 1, r8vals, 2, -5, nlv, 'SWDE A2CFDFS' )
  if ( ( index( errstr(1:errstr_len), 'UFBRMS - 5th ARG. (INPUT) IS .LE. 0' ) .eq. 0 ) ) stop 4
  ...
  if ( ( nlv .ne. 5 ) .or. &
      ...
      ( nint(r8vals(2,4)*10000) .ne. 7413 ) .or. ( nint(r8vals(2,5)*10000) .ne. 4968 ) ) stop 5
  ...
  if ( ( cmgtag .ne. 'NC008032' ) .or. ( idate .ne. 22053116 ) .or. ( nlv .ne. 1 ) .or. &
      ( nint(r8vals(1,1)*100000) .ne. 4081139 ) .or. ( nint(r8vals(2,1)*100000) .ne. -7787666 ) ) stop 6

  ! Copy the 3rd message to the output file.
  call rdmemm ( 3, cmgtag, idate, ier )
  if ( ( ier .ne. 0 ) .or. ( cmgtag .ne. 'NC002104' ) ) stop 7
  call cpymem ( 50 )
  ...
  if ( ireadmg ( 21, cmgtag, idate ) .ne. 0 ) stop 8
  ...
  if ( ( iostat1 .ne. 0 ) .or. ( iostat2 .ne. 0 ) ) stop 10
